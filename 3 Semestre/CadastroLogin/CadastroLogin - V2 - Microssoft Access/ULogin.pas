...
If (not (Dm.TbUsuarios.Locate('Apelido', FrmLogin.EdtApelido.Text,[loPartialKey ])) or  (Dm.TbUsuariosSenha.Value <> FrmLogin.EdtSenha.Text)) then
 begin
    MessageDlg('Nome ou senha do usuário inválidos.'+#13+ 'Se você esqueceu sua senha,'+#13+'consulte o administrador do sistema' +#13+ 'ou recupere sua senha ao lado >>', mtError, [mbOK], 0);
...
begin
key:=#0;
  If MessageDlg('Você Realmente Deseja Terminar a Aplicação?',mtCustom,[mbYes,mbNo],0)=mrYes then
  begin
...
//      FrmLogin.Left:=380;
      RecupSenha.Caption:='<<';
      Edit2.SetFocus;
      FrmLogin.Caption:='Esqueceu sua Senha! Tente a Recuperação de Senha do Sistema >>';
    end
    else
    begin
      EdtSenha.SetFocus;
...
begin
    If not (Dm.TbUsuarios.Locate('Nome', Edit2.Text,[loCaseInsensitive])) then
      begin
      Label8.Caption:='Usuário não Encotrado!';
      Label9.Caption:=''
      end
    else
    begin
